...
import{api,type MagazineGroup,type MagazineJob,type MagazinePlan,type MagazineUploadPackage,type MagazineVideo}from"./api";
...
export function YoutubeMagazineApp(){
 const[jobs,setJobs]=useState<MagazineJob[]>([]),[videos,setVideos]=useState<MagazineVideo[]>([]),[groups,setGroups]=useState<MagazineGroup[]>([]),[busy,setBusy]=useState(false),[error,setError]=useState(""),[notice,setNotice]=useState(""),[preview,setPreview]=useState<MagazinePlan|null>(null),[previewJob,setPreviewJob]=useState<string|null>(null),[uploadPackage,setUploadPackage]=useState<MagazineUploadPackage["package"]|null>(null);
 ...
 const prepareUpload=async(id:string)=>{setBusy(true);setError("");setNotice("");try{const result=await api.prepareMagazineUpload(id);setUploadPackage(result.package);await load();setNotice("비공개 업로드 메타데이터 패키지 생성 완료")}catch(e){setError(e instanceof Error?e.message:"업로드 패키징 실패")}finally{setBusy(false)}};
 return <div className="ymApp">
  ...
  {uploadPackage&&<section className="ymUploadPackage"><small>UPLOAD PACKAGE · PRIVATE</small><h2>{uploadPackage.metadata.snippet.title}</h2><p>API 업로드 가능: <b>{uploadPackage.readyForApiUpload?"YES":"NO"}</b> · 차단 사유: {uploadPackage.blockers.join(", ")}</p><div><span>SHA-256</span><code>{uploadPackage.video.sha256}</code></div><div><span>태그</span><b>{uploadPackage.metadata.snippet.tags.join(" · ")}</b></div><details><summary>설명 및 출처 미리보기</summary><pre>{uploadPackage.metadata.snippet.description}</pre></details></section>}
